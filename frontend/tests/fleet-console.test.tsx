...
describe("SHA fleet control plane", () => {
  it("supports operator search and renders reachable endpoint detail links", () => {
    ...
    fireEvent.change(search, { target: { value: "build-lnx-01" } })

    expect(screen.getByRole("link", { name: /open endpoint build-lnx-01/i })).toHaveAttribute(
      "href",
      "/endpoints/ep_demo_linux_01",
    )
    expect(screen.queryByText("eng-win-17")).not.toBeInTheDocument()
  })
})

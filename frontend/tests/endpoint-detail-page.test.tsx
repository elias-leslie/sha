...
describe("SHA endpoint detail route", () => {
  ...
  it("refreshes the shell title when a fixture endpoint has a newer live hostname", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("heading", { level: 1, name: /endpoint build-lnx-01-live/i })).toBeInTheDocument()
      expect(screen.getByRole("heading", { level: 2, name: /endpoint build-lnx-01-live/i })).toBeInTheDocument()
    })
  })

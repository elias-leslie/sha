...
def control_payload(
    *,
    ...
    if mappings is None:
        mappings = clean_mappings(sp80053_ids=["AC-17"])
    ...
    if controls is None:
        controls = [
            ...
            control_payload(
                ...
                mappings=clean_mappings(nist_csf_ids=["PR.AA-01"], sp80053_ids=["CM-2"]),
            ),
        ]
...
def test_source_pack_control_contract_enforces_formulas_and_nested_field_shape() -> None:
    ...
    bad = deepcopy(payload)
    bad["mappings"]["sp80053_ids"] = ["CM-2", "AC-17"]
    with pytest.raises(ValidationError):
        SourcePackControl.model_validate(bad)

    bad = deepcopy(payload)
    bad["mappings"]["sp80053_ids"] = ["AC-17", " AC-17"]
    with pytest.raises(ValidationError):
        SourcePackControl.model_validate(bad)
...
def test_source_pack_contract_accepts_public_source_families(source_family: str) -> None:
    ...
    assert model.source_family.value == source_family
    assert model.summary == "Curated starter pack for Example Starter Pack."
